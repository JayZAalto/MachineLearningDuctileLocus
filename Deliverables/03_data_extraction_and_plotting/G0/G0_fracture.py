...
df_Experi = pd.read_csv("G0_Expri.csv")
fdFrame = df_FEM['X']
...
if index != -1:
    ...
    plt.plot(X_Experi, y_Experi, '--o', color = 'black', markevery=35)
    ...
    plt.ylim([0,9.5])
    plt.xlim([0,2.3]) 
    plt.title("G0 FEM ML model 4 vs Experimental") 
    plt.savefig("G0 FEM ML model 4 vs Experimental")
    ...
    plt.xlabel("Displacement mm")
    plt.ylabel("Force N")
    plt.title("G0 Damage simulation ML predicted vs Experimental") 
    plt.savefig("G0 Damage simulation ML predicted vs Experimental")
    with open('fracture.csv', 'a') as f:
        writer = csv.writer(f)
        ps1 =[0, 0, 0, 0, 0, 0, 0, 0]
        writer.writerow(ps1)
