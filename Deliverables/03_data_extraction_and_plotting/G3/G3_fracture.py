...
triax = ls_sdv['Unnamed: 8'].astype(float)
loadAngle = ls_sdv['Unnamed: 9'].astype(float)
...
df_Experi = pd.read_csv("G3_Expri.csv")
fdFrame = df_FEM['X']
...
if index != -1:
    ...
    for x in fdFrame:
        if(x == frame[index]):
            ...
            break;
        i+=1
    with open('fractureFD_G3.csv', 'a') as f:
        writer = csv.writer(f)
        ps1 =[X[num], y[num]]
        writer.writerow(ps1)

    a = ls_sdv.drop(['Unnamed: 0','Unnamed: 5','Unnamed: 6','Unnamed: 7','Unnamed: 10'], axis=1)
    a.to_csv('fractureLS_'+pname+'.csv', sep=',')

    plt.plot(X_Experi, y_Experi, '--o', color = 'black', markevery=27)
    ...
    plt.ylabel("Force N")
    plt.title("G3 FEM ML model 4 vs Experimental") 
    plt.savefig("G3 FEM ML model 4 vs Experimental")
    ...
    plt.xlabel("Displacement mm")
    plt.ylabel("Force N")
    plt.title("G3 Damage simulation "+pname+ " vs Experimental") 
    plt.savefig("G3 Damage simulation "+pname+ " vs Experimental")
